...
- [ ] Add `test@example.com` to `ALLOWED_EMAILS` in `.dev.vars` (local Wrangler env only)
- [ ] Create a test-only API route (e.g. `GET /api/test/login`) that is conditionally registered only when `NODE_ENV === "test"` or a `CYPRESS` env flag is set; the route inserts a session row into D1 and returns the session ID
- [ ] Define `cy.loginAsTestUser()` in `cypress/support/commands.ts` — calls `GET /api/test/login`, then sets the `session_id` cookie
- [ ] Add TypeScript types for the custom command in `cypress/support/index.d.ts`
- [ ] Write a smoke test (`cypress/e2e/auth/login-stub.cy.ts`) that calls `cy.loginAsTestUser()` and asserts `/finance` renders without redirecting

## Done When

- [ ] `cy.loginAsTestUser()` is callable in any spec
- [ ] The smoke test passes against a running local Wrangler dev server
- [ ] The test endpoint is not reachable in a production build
